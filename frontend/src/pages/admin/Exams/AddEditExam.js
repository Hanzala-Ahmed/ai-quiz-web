...
function AddEditExam() {
  ...
  const generateByAi = async () => {
    try {
      dispatch(ShowLoading());
      const response = await generateQuestionsByAi(id);
      dispatch(HideLoading());
      if (response.success) {
        message.success(response.message);
        getExamDataById(id);
  ...
  return (
    <div>
      ...
      {(examData || !id) && (
        <Form layout='vertical' onFinish={onFinish} initialValues={examData} className='mt-2'>
          <Tabs defaultActiveKey='1'>
            <Tabs.TabPane tab='Exam Details' key='1'>
              <Row gutter={[10, 10]}>
                ...
                <Col span={8}>
                  <Form.Item label='Total Questions' name='totalQuestions'>
